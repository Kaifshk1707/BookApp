...
const Profile = () => {
  ...
  const endPointURL = "https://67c9694e0acf98d0708a2b66.mockapi.io/electronic";
...
const getApiData = async () => {
  ...
  const handleUpdateBook = async (id) => {
    ...
    setLoading(true)
    try {
      await axios.put(`${endPointURL}/${id}`, body)
    ...
    >
      ...
      >
        ...
        >
          ...
          <TouchableOpacity onPress={()=> !loading && handleUpdateBook(item.id)} style={{ paddingHorizontal: 15, paddingVertical: 8 }}>
            <Text style={{ fontSize: 16, fontWeight: "bold", color: "#555" }}>
              📖 
          ...
          <TouchableOpacity onPress={() => !loading && handleDeletePost(item.id)}>
            <Text style={{ fontSize: 16, fontWeight: "bold", color: "#555" }}>
